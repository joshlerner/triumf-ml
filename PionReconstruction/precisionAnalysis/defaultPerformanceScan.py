...
parser.add_argument('cells', type=int, help='the number of cells in the model input')
...
with suppress_stdout():
    ...
    model = tf.keras.models.load_model(out_path + f'models/GarNet_log_{args.cells}')
    qmodel = tf.keras.models.load_model(out_path + f'models/qGarNet_log_{args.cells}')

    test_generator = garnetDataGenerator(test_file_list,
                                         cell_geo_path,
                                         batch_size=20000,
                                         normalizer=('log', None),
                                         name=f'garnet_log_{args.cells}',
                                         labeled=True,
                                         preprocess=False,
    ...
    data[args.name][()][args.precision]['Continuous'][f'{args.cells}'] = s
    ...
    hls_scaled_pred = np.exp(hls_model.predict(x)[-1]*10).reshape(-1,)
    s = np.nan_to_num(Norm/rms(hls_scaled_pred, scaled_target), nan=0.0, neginf=0.0, posinf=0.0)
    data[args.name][()][args.precision]['Quantized'][f'{args.cells}'] = s
    np.savez(precisionFile, **data)

data = np.load(precisionFile, allow_pickle=True)
data = dict(data)
print(f'{args.precision} - {args.cells}:')
print(f"\t Continuous: {data[args.name][()][args.precision]['Continuous'][f'{args.cells}']} \
        \t Quantized: {data[args.name][()][args.precision]['Quantized'][f'{args.cells}']}")
